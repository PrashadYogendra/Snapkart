...
import { decreaseQuantity, increaseQuantity, removeFromCart } from '@/redux/cartSlices'

function CartPage() {
    const {cartData, subTotal, deliveryCharges, finalTotal} = useSelector((state:RootState) => state.cart)
    const dispatch = useDispatch<AppDispatch>()
  return (
    <div className='w-[-95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative'>
        <Link href={"/"} className='absolute -top-2 left-0 flex items-center gap-2 text-green-700
                        ...
                        {cartData.map((item,index) => (
                            ...
                            >
                                ...
                                rounded-xl overflow-hidden bg-gray-50'>
                                    <Image
                                    src={item.image} alt={item.name} fill className='object-contain p-3 transition-transform duration-300 hover:scale-105' />
                                </div>
                                <div className='mt-4 sm:mt-0 sm:ml-4 flex-1 text-center sm:text-left'>
                                    ...
                                    <p className='text-sm font-bold text-green-700 mt-1 sm:text-base'>₹{(Number(item.price) * item.quantity)}</p>
                                </div>
                ...
                flex-col'>
                    <h2 className='text-lg sm:text-xl font-bold text-gray-800 mb-4'>
                        ...
                        <div className='space-y-3 text-gray-700 text-sm sm:text-base'>
                            ...
                            <span className='text-green-700 font-semibold'>₹{deliveryCharges}</span>
                            </div>
